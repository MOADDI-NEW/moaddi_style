...
if (isset($_SESSION['Edara30'])){
    $pageTitle = 'شركات مرشحة من الموسوقين';  // this function to load page title
	...
	if (array_search($info['role'], ['30']) !== false) { //   ادارة الجيم 
      ...
      if($do == 'Manage'){  //==== Manage Page == 
			if (array_search($info['GroupID'], ['1','3']) !== false) {	   // مدير + مساعد
				$stmt = $con->prepare("SELECT c.*, u.UserID, u.FullName, u.country, u.phone, u.role, u.RegStatus
                                          FROM my_companies c
                                          JOIN users u ON u.UserID = c.Member_ID
                                          WHERE u.role = 45 AND u.RegStatus = 1 ORDER BY u.UserID DESC");
				$stmt->execute();
				$rows = $stmt->fetchAll();
				if (! empty($rows)){ ?>
					<div class="container-fluid">
						<div class="row row-sm">
							<div class="col-lg-12">
								<div class="card">
									<div class="card-header"><h3 class="card-title text-center">  شركات مرشحة من الموسوقين </h3></div>
									<div class="card-body">
										<div class="table-responsive export-table">
											...
											<thead>
												<tr>
													<th> #</th>
													<th class="bg-info" > المسوق</th>
													<th class="bg-info" > دولته</th>
													<th class="bg-info" > رقم الهاتف</th>
														...
														echo "<tr>";
															...
															echo "<td data-label='التحكم' style='text-align:center;'>";
																if (array_search($info['GroupID'], ['1']) !== false) { // المدير
																		echo"<a href='compnies_from_makting?memberid=315&formerror=form&action=employee&emplyeeid=3265&action=grtstat&grtstat=359&formerror=9853&getid=63524&iteimid=32145&checksum=56821&cookie=30121&do=Print&userid=" . $row['Item_ID'] ."&counksum=93214&action=421' class=''><i class='fas fa-print pl-2' style='text-decoration: none;color:#0c0ff3;font-size:15px;'></i>  </a>";
																		echo "<a href='compnies_from_makting?do=Delete&userid=" . $row['Item_ID'] . "' class='deleteEmployee' title=\" حذف\"  ><i class='far fa-trash-alt' style='text-decoration: none;color: crimson;padding: 5px;'></i> </a>";
																}
															echo "</td>";
															echo "<td data-label='التحكم' style='text-align:center;'>";
																if (array_search($info['GroupID'], ['1']) !== false) { // المدير
                                                   if ($row['Approve'] == 0 ) {
                                                    	echo"<a href='compnies_from_makting?memberid=315&formerror=form&action=employee&emplyeeid=3265&action=grtstat&grtstat=359&formerror=9853&getid=63524&iteimid=32145&checksum=56821&cookie=30121&do=Approve&userid=" . $row['Item_ID'] ."&counksum=93214&action=421' class=''> موافقة </a>   ";
                                                   } else{
                                                      echo "تمت الموافقة";
                                                   }
																}
						...
						<div class="container-fluid" style="direction:ltr !important;">
							<div class="form-style-10 px-2 py-2" style="background:repeating-linear-gradient(#f8efde38,#f8efde0d 20px,#f8efde00 20px,#e5d0a2 22px ); border: 10px dotted rgb(0 0 0 / 18%);">
									...
									<div class="row">
										...
										<div class="col-12">
										<div class="section"> Companiey from Markting </div>
                                    ...
                                    <div class="row d-flex justify-content-center">
                                       <?php 
                                        ...
                                        <div class="col-3">
                                          <?php echo '<img src="../admin/nsharat_uploads/products_img/' . trim($image) . '" alt="Product Image" class="img-rounded img-thumbnail" style="width:150px;">'; ?>
                                        </div><?php
			...
			if (array_search($info['GroupID'], ['1']) !== false) {	  
					...
					echo "<div class ='container'>";
						$userid = isset($_GET['userid'])&& is_numeric($_GET['userid']) ? intval($_GET['userid']) : 0;
						// Check if the Userid is numeric and  Exist in Database	
						$check = checkItem ('Item_ID', 'my_companies', $userid);
						if ($check > 0){ 
							$stmt = $con->prepare("DELETE FROM my_companies WHERE Item_ID = :zuser");
			...
			// Check if the Userid is numeric and  Exist in Database	
					$check = checkItem ('Item_ID', 'my_companies', $userid);
					if ($check > 0){ 
						$stmt = $con->prepare("UPDATE my_companies SET Approve = 1 WHERE Item_ID = ?");
						$stmt->execute(array($userid));
						if ($stmt) { // if it's true
							sleep(1);?>
