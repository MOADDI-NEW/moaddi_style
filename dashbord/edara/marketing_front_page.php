...
if (isset($_SESSION['Edara30'])){
    $pageTitle = 'صفحة الاعدادات لصفحة المسوقين ';  // this function to load page title
	...
	if (array_search($info['role'], ['30']) !== false) { //   ادارة الجيم 
		...
		if($do == 'Manage'){  //==== Manage Page == 
				...
				if (! empty($rows)){ ?>
					<div class="container-fluid">
						<div class="row row-sm">
							<div class="col-lg-12">
								<div class="card">
									<div class="card-header"><h3 class="card-title text-center"> اعدادات  صفحة المسوقين</h3></div>
									<div class="card-body">
			...
			$stmt = $con->prepare("SELECT * FROM marketing_front_page WHERE id = ?  LIMIT 1");
			...
			if ($stmt->rowCount() > 0 ){ ?>
					<div class="container-fluid">
						<div class="row row-sm">
							<div class="col-lg-12">
									<div class="card">
										<div class="card-header"><h3 class="card-title text-center"> تعديل اعدادات  صفحة المسوقين</h3></div>
										<div class="card-body">
			...
			if($_SERVER['REQUEST_METHOD'] == 'POST'){
					...
					if (empty($formErrors)){
							...
							$stmt = $con->prepare("UPDATE marketing_front_page SET 
																		text_page = ?  WHERE id = ?");
