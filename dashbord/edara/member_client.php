...
if (isset($_SESSION['Edara30'])){
    $pageTitle = 'صفحة أعضاء العائلة ';  // this function to load page title
	...
	if (array_search($info['role'], ['30']) !== false) { //   ادارة الجيم 
        ...
        if($do == 'Manage'){  //==== Manage Page == 
            if (array_search($info['GroupID'], ['1','3']) !== false) {	   // مدير + مساعد
                $stmt = $con->prepare("SELECT * FROM users WHERE role = 44 AND RegStatus = 1 ORDER BY UserID DESC");
                $stmt->execute();
                $rows = $stmt->fetchAll();
                if (! empty($rows)){ ?>
                    <div class="container-fluid">
                        <div class="row row-sm">
                            <div class="col-lg-12">
                                <div class="card">
                                    <div class="card-header"><h3 class="card-title text-center">قوائم أعضاء العائلة</h3></div>
                                        <?php
                                            if (isset($_SESSION['Edara30'])){
                                                $getUser = $con->prepare("SELECT * FROM users WHERE Username = ?");
                                                $getUser->execute(array($_SESSION['Edara30']));
                                            ...
                                            <a href="member_client?do=Add" class="btn btn-primary"><i class="fa fa-plus"></i> اضافة عضو جديد </a>
                                            <?php
                                                }
                                            }
                                        ?>
                                                ...
                                                <tbody>
                                                    ...
                                                    echo "<tr>";
                                                        ...
                                                        echo "<td data-label='التحكم' style='text-align:center;'>";
                                                            if (array_search($info['GroupID'], ['1']) !== false) { // المدير
                                                                echo"<a href='member_client?memberid=315&formerror=form&action=employee&emplyeeid=3265&action=grtstat&grtstat=359&formerror=9853&getid=63524&iteimid=32145&checksum=56821&cookie=30121&do=Edit&userid=" . $row['UserID'] ."&counksum=93214&action=421' class=''><i class='fas fa-user-edit' style='text-decoration: none;color:#0c0ff3;font-size:15px;'></i>  </a>";
                                                                echo "<a href='member_client?do=Delete&userid=" . $row['UserID'] . "' class='deleteEmployee' title=\" حذف\"  ><i class='far fa-trash-alt' style='text-decoration: none;color: crimson;padding: 5px;'></i> </a>";
                                                            }
                                                        echo "</td>";
                                                        echo "<td data-label='التحكم' style='text-align:center;'>";
                                                            if (array_search($info['GroupID'], ['1']) !== false) { // المدير
                                                                echo"<a href='member_client?memberid=315&formerror=form&action=employee&emplyeeid=3265&action=grtstat&grtstat=359&formerror=9853&getid=63524&iteimid=32145&checksum=56821&cookie=30121&do=DeRegStatus&userid=" . $row['UserID'] ."&counksum=93214&action=421' class=''> تعطيل </a>  -- ";
                                                                echo"<a href='member_client?memberid=315&formerror=form&action=employee&emplyeeid=3265&action=grtstat&grtstat=359&formerror=9853&getid=63524&iteimid=32145&checksum=56821&cookie=30121&do=Death&userid=" . $row['UserID'] ."&counksum=93214&action=421' class=''> وفاه </a>";
                                                            }
                                                        echo "</td>";
                                                    echo "</tr>";
                                                    } 							
